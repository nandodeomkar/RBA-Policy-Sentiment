...
> **Status — 2026-06-08:** **Planned; build not started.** Implementation plan written — [M3 implementation plan](../../superpowers/plans/2026-06-08-rba-m3-implementation-plan.md) (from the [M2/M3 design](../../superpowers/specs/2026-06-08-rba-m2-m3-design.md) §8–§9), on `feat/m2-m3`. Approach: **FR-010 first** (date-zoom slider + shareable-URL query state + pure `encodeViewState`/`decodeViewState` helpers; the dataZoom window replaces Year-only windowing, Year becomes a shortcut; table stays in lockstep) — **frontend-only, fully autonomous**. Then **FR-012** — an offline, **cached, descriptive-only** LLM tone summary (`tone_summary` + `tone_summary_version` added to `scores.json`, **versioned separately** from `engine_version`, backward-compatible; deterministic `buildHeadline` stays the fallback). FR-012 needs a **one-time live LLM run** to populate (~64 cached calls); all code + the frontend build/test without a key. No launch-gate change (NFR-004 untouched).
